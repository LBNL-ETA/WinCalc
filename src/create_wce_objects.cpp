...
namespace wincalc
{
    ...
    SpectralAveraging::MeasuredRow convert(OpticsParser::WLData const & data)
    {
        if(!data.directComponent.has_value() && !data.diffuseComponent.has_value())
        {
            ...
            msg << "Wavelength " << data.wavelength
                << " does not have either a direct or diffuse measurement";
      ...
      convert(std::vector<std::tuple<double, double, double, double>> const & measured_data)
    {
        ...
        {
            spectral_data->addRecord(std::get<0>(val),
                                     std::get<1>(val),
                                     std::get<1>(val),
                                     std::get<2>(val),
                                     std::get<3>(val));
      ...
      create_pv_material(wincalc::Product_Data_N_Band_Optical const & product_data)
    {
        ...
        auto pvSample =
          std::make_shared<SpectralAveraging::PhotovoltaicSampleData>(*sample, eqef, eqeb);
      ...
      SingleLayerOptics::BSDFHemisphere const & bsdf_hemisphere)
    {
        ...
        auto layer = SingleLayerOptics::CBSDFLayerMaker::getDirDifLayer(material, bsdf_hemisphere);
        LOGMSG("begin create_bsdf_layer_direct_diffuse(product_data, " + method.name + ")");
        return layer;
    }
      ...
      SingleLayerOptics::BSDFHemisphere const & bsdf_hemisphere)
    {
        ...
        {
            return create_bsdf_layer_specular(
              product_data, method, number_of_layers, bsdf_hemisphere);
        }

        return create_bsdf_layer_direct_diffuse(
          product_data, method, number_of_layers, bsdf_hemisphere);
    }
      ...
      int number_solar_bands)
    {
        ...
        for(auto product : product_data)
        {
            ...
            {
                if(!nband->is_specular_only())
                {   // <- replaces the inner wavelength loop
                    as_bsdf = true;
                    break;
                }
            }
