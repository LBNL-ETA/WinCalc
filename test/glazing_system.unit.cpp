...
TEST_F(TestGlazingSystem, Test_NFRC_102_2011_SA1_same_solar_and_visible_glazing_system)
{
	SCOPED_TRACE("Begin Test: Double layer NFRC 102-2011-SA1 but same solar and visible values.");
	...
	bsdf_path /= "products";
	bsdf_path /= "2011-SA1_same_solar_and_visible.XML";
	...
	auto u_result = glazing_system_u.u();
	EXPECT_NEAR(u_result, 3.13349342, 1e-6);
	...
	auto shgc_result = glazing_system_shgc.shgc();
	EXPECT_NEAR(shgc_result, 0.09319861, 1e-6);
	...
	WCE_Optical_Results solar_results =
		glazing_system_u.optical_method_results(Optical_Standard_Method_Type::SOLAR);
	EXPECT_NEAR(
		solar_results.system_results.front.transmittance.direct_hemispherical, 0.0166, 1e-4);
	EXPECT_NEAR(
		solar_results.system_results.front.reflectance.direct_hemispherical, 0.7091, 1e-4);
	EXPECT_NEAR(
		solar_results.system_results.back.reflectance.direct_hemispherical, 0.5304, 1e-4);
#if 0
	EXPECT_NEAR(
		solar_results.system_results.front.transmittance.direct_direct, 0.83384927443226908, 1e-6);
	EXPECT_NEAR(
	...
	WCE_Optical_Results photopic_results =
		glazing_system_u.optical_method_results(Optical_Standard_Method_Type::PHOTOPIC);
	EXPECT_NEAR(
		photopic_results.system_results.front.transmittance.direct_hemispherical, 0.0168, 1e-4);
#if 0
	EXPECT_NEAR(
		photopic_results.system_results.front.transmittance.direct_direct, 0.89925998453884770, 1e-6);
	EXPECT_NEAR(
