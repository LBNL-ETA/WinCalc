...
class TestNFRC6046 : public testing::Test
{
protected:
	...
	virtual void SetUp()
	{
		...
		glazing_system_shgc =
			std::make_shared<Glazing_System>(products, gaps, standard, 1.0, 1.0, nfrc_shgc_environments());
	}
};

TEST_F(TestNFRC6046, Test_U)
{
	auto u_result = glazing_system_u->u();
	EXPECT_NEAR(u_result, 3.2409776, 1e-6);
}

TEST_F(TestNFRC6046, Test_SHGC)
{
	auto shgc_result = glazing_system_shgc->shgc();
	EXPECT_NEAR(shgc_result, 0.434647, 1e-6);
}

TEST_F(TestNFRC6046, Test_Solar)
{
	WCE_Optical_Results solar_results = glazing_system_u->optical_method_results(Optical_Standard_Method_Type::SOLAR);
	EXPECT_NEAR(solar_results.system_results.front.transmittance.direct_direct, 0.38320977, 1e-6);
	EXPECT_NEAR(solar_results.system_results.front.transmittance.direct_hemispherical, 0.38320977, 1e-6);
	EXPECT_NEAR(solar_results.system_results.back.transmittance.direct_direct, 0.38320977, 1e-6);
	EXPECT_NEAR(solar_results.system_results.front.reflectance.direct_direct, 0.28597167459, 1e-6);
	EXPECT_NEAR(solar_results.system_results.back.reflectance.direct_direct, 0.4492397297, 1e-6);
	...
	EXPECT_NEAR(solar_results.system_results.front.transmittance.diffuse_diffuse, 0.335087587, 1e-6);
	EXPECT_NEAR(solar_results.system_results.back.transmittance.diffuse_diffuse, 0.335087587, 1e-6);
	EXPECT_NEAR(solar_results.system_results.front.reflectance.diffuse_diffuse, 0.32151683716764318, 1e-6);
	EXPECT_NEAR(solar_results.system_results.back.reflectance.diffuse_diffuse, 0.47433071908884489, 1e-6);
}

TEST_F(TestNFRC6046, Test_Photopic)
{
	WCE_Optical_Results photopic_results = glazing_system_u->optical_method_results(Optical_Standard_Method_Type::PHOTOPIC);

	EXPECT_NEAR(photopic_results.system_results.front.transmittance.direct_direct, 0.79206238819268404, 1e-6);
	EXPECT_NEAR(photopic_results.system_results.back.transmittance.direct_direct, 0.79206238819268404, 1e-6);
	EXPECT_NEAR(photopic_results.system_results.front.reflectance.direct_direct, 0.060225213828750658, 1e-6);
	EXPECT_NEAR(photopic_results.system_results.back.reflectance.direct_direct, 0.046788445237731444, 1e-6);
	...
	EXPECT_NEAR(photopic_results.system_results.front.transmittance.diffuse_diffuse, 0.7157183939, 1e-6);
	EXPECT_NEAR(photopic_results.system_results.back.transmittance.diffuse_diffuse, 0.7157183939, 1e-6);
	EXPECT_NEAR(photopic_results.system_results.front.reflectance.diffuse_diffuse, 0.1253425452, 1e-6);
	EXPECT_NEAR(photopic_results.system_results.back.reflectance.diffuse_diffuse, 0.11298229009427586, 1e-6);
}

TEST_F(TestNFRC6046, Test_SPF)
{
	EXPECT_THROW(glazing_system_u->optical_method_results(Optical_Standard_Method_Type::SPF), std::runtime_error);

}

TEST_F(TestNFRC6046, Test_TDW)
{
	WCE_Optical_Results tdw_results = glazing_system_u->optical_method_results(Optical_Standard_Method_Type::TDW);

	EXPECT_NEAR(tdw_results.system_results.front.transmittance.direct_direct, 0.57819767895463237, 1e-6);
	EXPECT_NEAR(tdw_results.system_results.back.transmittance.direct_direct, 0.57819767895463237, 1e-6);
	EXPECT_NEAR(tdw_results.system_results.front.reflectance.direct_direct, 0.07867988484307524, 1e-6);
	EXPECT_NEAR(tdw_results.system_results.back.reflectance.direct_direct, 0.11999639709151914, 1e-6);
	...
	EXPECT_NEAR(tdw_results.system_results.front.transmittance.diffuse_diffuse, 0.52246732115715033, 1e-6);
	EXPECT_NEAR(tdw_results.system_results.back.transmittance.diffuse_diffuse, 0.52246732115715033, 1e-6);
	EXPECT_NEAR(tdw_results.system_results.front.reflectance.diffuse_diffuse, 0.1423186835969221, 1e-6);
	EXPECT_NEAR(tdw_results.system_results.back.reflectance.diffuse_diffuse, 0.18032504130534477, 1e-6);
}

TEST_F(TestNFRC6046, Test_TKR)
{
...
TEST_F(TestNFRC6046, Test_Color)
{
